...
def inverse_laplace_transform(equation, rate_by_mutype, split_time):
    ...
    process = subprocess.run(["giac", invlaplace_string], stderr=subprocess.DEVNULL, stdout=subprocess.PIPE, encoding='utf-8')
    probability = process.stdout.rstrip("\n").split(",")[1]
    #print(probability)
    return sympy.Rational(str(probability))
