...
def inverse_laplace_transform(params):
    ...
    if probability == 'undef':
        print(giac_string, probability, vector, marginal_query)
    try:
...
class PathObj(object):
    ...
    def infer_mutation_equation(self, mutuple, parameterObj):
        ...
        #print("mutuple: %s %s" % (mutuple, mutation_equation))
        return mutation_equation
            ...
            if parameterObj.threads < 2:
                for param in params:
                    ...
                    if elapsed > longest_time:
                        ...
                        heaviest = vector
                    print(vector, "\t", probability, "\t", "%s s" % (timer() - start))
            else:
...
def build_equation(args):
    ...
    #print("# Mutation eq", mutuple, mutation_equation)
