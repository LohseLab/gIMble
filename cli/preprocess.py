"""usage: gimbl preprocess               -f FILE -v FILE -b DIR [-g INT -m INT -M INT -q INT -t INT -o STR -k] [-h|--help]
...
- let user know
- any sort of improvement  would brake vcf compatibiliity

The purpose of this module is to preprocess the genotype data by parsing input files, applying necessary quality filters, and writing output files. 
The output files generated by gimbl preprocess serve as input for subsequent gimble modules.
- problem of additional user filters
    - explain in docs, explain possibility of filtering post, but at own risk


When should users specify additional filtering steps? 
