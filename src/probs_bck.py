...
class ParameterObj(object):
    def __init__(self, args):
        ...
        self.user_rate = {
            ...
            'T' : float(args['--T_var']),
            'theta' : float(args['--mutation_rate'])
        }
        self.base_rate = {
            var('M') : float(args['--migration_rate']) / 2,
            var('C_ancestor') : 1.0,
            var('C_derived') : float(args['--derived_coalescence_rate']),
            var('E') : var('E', domain='positive')
        }
        self.split_time = float(args['--T_var'])
        self.exodus_rate = var('E', domain='positive')
        self.mutation_rate = float(args['--mutation_rate']) / 2
    ...
    def calculate_probabilities(self, x0, *args):
        ...
        #print("[+] Calculating probabilities (datapoints and marginals)...")
        # unpack parameters
        PoD, data, symbolic_equations_by_data_idx, base_rate, mutation_rate, split_time = args
        split_time = x0[0]
        mutation_rate = x0[1]
        ...
        for mutypes in reversed(sets_of_mutation_rates):
            ...
            for data_idx, equation in equation_by_data_idx.items():                
                ...
                if not mutypes_masked: 
                    if not data_string in data_queries_seen:
                        probability_ilt = inverse_laplace(equation.substitute(rate_by_mutype) / var('E'), var('E'), var('T'), algorithm='sympy').substitute(T=split_time)
                        prob_by_data_string[data_string] = probability_ilt
        ...
        #print("[=] Calculated probabilities in %s seconds..." % (timer() - start_time))
        composite_likelihood = -np.sum((xlogy(np.sign(A), A) * PoD))
        print(composite_likelihood, 'T=%s, mutation_rate=%s, M=%s, C_ancestor=%s, C_derived=%s' % (split_time, mutation_rate, base_rate[var('M')], base_rate[var('C_ancestor')], base_rate[var('C_derived')]))
        return composite_likelihood
...
class BoundsObj(object):
    def __init__(self, xmax=[2.0, 2.0], xmin=[0.1, 3.0] ):
...
def main():
    try:
        ...
        composite_likelihood_truth, PoD = parameterObj.calculate_initial_probabilities(data, symbolic_equations_by_data_idx)
        x0 = (2.0, 0.1) # T, theta
        #args = (PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time)
        #from scipy.optimize import fmin
        #ret = fmin(parameterObj.calculate_probabilities, x0, args)
        #from scipy.optimize import basinhopping
        #minimizer_kwargs = {"method": "BFGS", "jac": False, "args": (PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time)}
        #bounds = BoundsObj()
        #x0 = (0.1, 4.0) # T, theta
        #ret = basinhopping(parameterObj.calculate_probabilities, x0, stepsize=0.1, minimizer_kwargs=minimizer_kwargs, niter=10, accept_test=bounds)
        #print(ret)
        
        #bounds = ((1.2, 1.6), (0.4, 0.8))
        initial_simplex = [(0.1, 2.0), (2.0, 0.1), (0.1, 0.1)]
        parameterObj.calculate_probabilities(x0, PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, 0.1, 2.0)
        exit()
        res = minimize(parameterObj.calculate_probabilities, x0, args=(PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time), method="Nelder-Mead", options={'initial_simplex': initial_simplex, 'disp': True, 'xatol': 1e-25, 'fatol': 1e-25, 'adaptive': True}) # does not works

        #res = minimize(parameterObj.calculate_probabilities, x0, args=(PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time), method="Nelder-Mead", options={'disp': True}) # does not works
        #res = minimize(parameterObj.calculate_probabilities, x0, args=(PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time), method="TNC", bounds=bounds, options={'eps': 0.1, 'disp': True}) # does not works
        #res = minimize(parameterObj.calculate_probabilities, x0, args=(PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time), method="SLSQP", bounds=bounds, options={'disp': True}) # does not works
        #res = minimize(parameterObj.calculate_probabilities, x0, args=(PoD, data, symbolic_equations_by_data_idx, parameterObj.base_rate, parameterObj.mutation_rate, parameterObj.split_time), method=custmin, options=dict(stepsize=0.1)) # works
        print(res)
        exit()
